...
private:
	...
	gles2::shader::program _text_prog;
	FT_Library _lib;
	FT_Face _face;
...
void label<Window>::render()
{
	...
	static gles2::mesh quad = gl::make_quad_xy<gles2::mesh>();
	quad.render();
}
...
//		pixel_format::red, pixel_type::ub8, blob.data());
	...
	_text_tex = gles2::texture2d(im.columns(), im.rows(),
		gles2::pixel_format::luminance, gles2::pixel_type::ub8, blob.data());
}
