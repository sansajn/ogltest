...
public:
	...
	gpu_buffer(buffer_target target, unsigned size, buffer_usage usage);
	gpu_buffer(buffer_target target, void const * buf, unsigned size, buffer_usage usage);
	gpu_buffer(gpu_buffer && other);
	~gpu_buffer();
	void data(buffer_target target, void const * buf, unsigned size, unsigned offset = 0);
	unsigned id() const;
