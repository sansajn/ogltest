...
public:
	...
	texture(unsigned width, unsigned height, internal_format ifmt, pixel_format pfmt, pixel_type type, void * data);
	texture(texture && lhs);
	~texture();

	void read(std::string const & fname);
	void read(unsigned width, unsigned height, internal_format ifmt, pixel_format pfmt, pixel_type type, void * data);
