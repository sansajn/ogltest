...
int const nindices = sizeof(indices)/sizeof(indices[0]);
int const nvertices = sizeof(vertices)/sizeof(vertices[0])/3;
...
*/
	...
	glBufferData(GL_ARRAY_BUFFER, 2*nvertices*3*sizeof(GLfloat), nullptr, GL_STATIC_DRAW);
	glBufferSubData(GL_ARRAY_BUFFER, 0, nvertices*3*sizeof(GLfloat), (GLvoid *)vertices);
	glBufferSubData(GL_ARRAY_BUFFER, nvertices*3*sizeof(GLfloat), nvertices*3*sizeof(GLfloat), (GLvoid *)normals);
	...
	glVertexAttribPointer(normal_attr_id, 3, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(nvertices*3*sizeof(GLfloat)));
