...
tweakbar_manager::tweakbar_manager(std::vector<bar_data> const & bars, bool minimized)
	: _twbar(nullptr), _bars(bars), _minimized(minimized), _initialized(false), _updated(true)
{
	for (auto & b : _bars)
		b.owner = this;
...
void tweakbar_manager::init(std::vector<bar_data> const & bars, bool minimized)
{
	_twbar = nullptr;
	_bars = bars;
