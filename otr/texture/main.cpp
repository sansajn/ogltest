...
public:
		...
		: sdl_window(parameters().size(800, 600).version(3, 3)), _fb(framebuffer::default_fb())
	{
		...
		*_p << "shader/minimal.fs";
		...
		unsigned char data[16] = {0, 255, 0, 255, 255, 0, 255, 0, 0, 255, 0,
			255, 255, 0, 255, 0};

		ptr<texture2D> tex(new texture2D(4, 4, texture_internal_format::R8,
			texture_format::RED, pixel_type::UNSIGNED_BYTE,
			texture::parameters().mag(texture_filter::NEAREST),
			buffer::parameters(), cpubuffer(data)));

		*_u = tex;

//		_fb.polygon_mode(polygon_raster_mode::LINE);
	}
