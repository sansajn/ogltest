...
void on_init()
{
	...
	glBufferData(GL_ARRAY_BUFFER, nverts*sizeof(glm::vec3), &verts[0], 
		GL_STATIC_DRAW);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
	glEnableVertexAttribArray(0);

	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, bufs[1]);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, ninds*sizeof(GLuint), &inds[0], 
		GL_STATIC_DRAW);
